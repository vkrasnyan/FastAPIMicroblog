...
class TestFollowUser:
    ...
    async def test_follow_and_unfollow(
        ...
        me_response = await async_client.get(f"{ROOT_ENDPOINT}/me", headers={"api-key": user1.api_key})
        data = me_response.json()
        assert any(f["id"] == user2.id for f in data["user"]["following"])

        # Проверка, что user2 имеет подписчика user1
        me_response_2 = await async_client.get(f"{ROOT_ENDPOINT}/me", headers={"api-key": user2.api_key})
